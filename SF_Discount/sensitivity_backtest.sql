...
/*
among DP new signup (trial, PM 1-3)
what is the price sens by P90D SP OF? by P28D DP savings
*/
...
select 
    be.*
 ,sum(case when is_subscribed_consumer = true and created_at::date between first_exposed::date - 28 and first_exposed::date - 1 then fee-delivery_fee+service_fee_no_dscnt-service_fee else 0 end) DP_savings_28D
 --,sum(case when is_subscribed_consumer = true and created_at::date between first_exposed::date - 90 and first_exposed::date - 1 then fee-delivery_fee+service_fee_no_dscnt-service_fee end) DP_savings_90D
  , count(distinct case when is_subscribed_consumer = true and created_at::date between first_exposed::date - 90 and first_exposed::date - 1 then delivery_id end) as l90d_dp_of
  --, count(distinct case when is_subscribed_consumer = true and created_at::date between first_exposed::date - 28 and first_exposed::date - 1 then delivery_id end) as l28d_dp_of
   from exp_first_order_date be
left join public.dimension_deliveries dd on be.user_id = dd.creator_id and created_at::date <= first_exposed::date - 1 and is_filtered_core = true
group by all
)
...
new_dp as
(SELECT
  dsa.consumer_id, dsa.user_id, dsa.dte
FROM
...
WHERE
  dsa.dte between $start_date::date - 7 and $end_date
...
, case
    --when is_new_cx = 1 then '0. New Cx'
    when l90d_dp_of = 0 then '1. O Order'
    when l90d_dp_of <= 5 then '2. 1-5 Orders'
    when l90d_dp_of <= 10 then '3. 6-10 Orders'
    when l90d_dp_of <= 20 then '4. 11-20 Orders'
    when l90d_dp_of <= 30 then '5. 20-30 Orders'
    when l90d_dp_of > 30 then '6. >= 30 Orders'
  end as l90d_dp_of_cohort
, case
    --when is_new_cx = 1 then '0. New Cx'
    when DP_savings_28D = 0 then '1. $0'
    when DP_savings_28D <= 500 then '2. $0-$5'
    when DP_savings_28D <= 999 then '3. $5-$9.99'
    when DP_savings_28D <= 2000 then '4. $9.99-$20'
    when DP_savings_28D <= 4500 then '5. $20-$45'
    when DP_savings_28D > 4500 then '6. >$45'
  end as DP_savings_28D_cohort
, c.*
from be a
left join core_dd c on c.creator_id = a.user_id AND c.CREATED_AT >=  a.first_exposed
left join new_dp x on x.user_id = a.user_id AND x.dte =  a.first_exposed
left join dp_adoption dpa on a.user_id = dpa.consumer_id 
)
...
order by DP_savings_28D_cohort
;
...
select 
   case 
     when tag like '%holdout%' then 'Control' 
     else 'Treatment'
     end 
    as tag_renamed
  , bucket_key as user_id 
...
where experiment_name = $exp_name
  and experiment_version between $exp_start_version and $exp_end_version
  --and convert_timezone('UTC','America/Los_Angeles',exposure_time) between '2021-02-01 20:00:00' and '2022-12-10 00:00:00'
 and tag in ('cali_control_holdout','cali_treatment_dp_sf_min_control_to_150')
 --and bucket_key != 'unset_bucket_value'
 and bucket_key not in ('1505155093') --Exclude Dashmart re-stocking cx_id
 --and SEGMENT in ('Users')
group by all
...
-- Overall Eligibile Cohort Sensitivity
set exp_start_version = 16;
set exp_end_version = 20;
set start_date = '2022-10-24'::date;
set end_date = '2023-09-23'::date;
...
, a.first_exposed
, dpa.consumer_id as dp_sign_up
, case when x.user_id is null then 'Others'
else 'New' end cohort
, c.*
from be a
left join core_dd c on c.creator_id = a.user_id AND c.CREATED_AT >=  a.first_exposed
