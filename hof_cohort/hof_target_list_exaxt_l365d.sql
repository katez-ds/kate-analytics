...
WITH
-- (1a) L365D + L180D + L28D orders from a single dimension_deliveries scan (all three windows
--      end on snapshot and nest inside one another, so one pass with conditional counts covers
--      all three), scoped to the US/Classic/tenured base cohort to bound the scan.
deliveries AS (
    SELECT
        dd.creator_id,
        COUNT(DISTINCT dd.delivery_id) AS l365d_orders,
        COUNT(DISTINCT CASE WHEN dd.created_at::DATE >= DATEADD('day', -179, $snap::DATE)
                             THEN dd.delivery_id END) AS l180d_orders,
        COUNT(DISTINCT CASE WHEN dd.created_at::DATE >= DATEADD('day', -27, $snap::DATE)
                             THEN dd.delivery_id END) AS l28d_orders
    FROM proddb.public.dimension_deliveries dd
    JOIN proddb.mattheitz.mh_customer_authority ca
      ON ca.creator_id = dd.creator_id
     AND ca.dte = $snap::DATE
    ...
    WHERE dd.is_filtered_core = TRUE
      AND dd.created_at::DATE BETWEEN DATEADD('day', -364, $snap::DATE) AND $snap::DATE
    GROUP BY 1
),

-- (1b) Cohort: US, Classic-only, tenured, L365D orders in [50, 100], @ snapshot.
cohort AS (
    SELECT
        ca.creator_id,
        'Classic' AS consumer_type,
        COALESCE(d.l365d_orders, 0) AS l365d_orders,
        COALESCE(d.l180d_orders, 0) AS l180d_orders,
        COALESCE(d.l28d_orders, 0) AS l28d_orders
    FROM proddb.mattheitz.mh_customer_authority ca
    LEFT JOIN deliveries d ON d.creator_id = ca.creator_id
    WHERE ca.dte = $snap::DATE
      AND ca.acquisition_country_id = 1
      AND ca.dp_sub_flag_start IS DISTINCT FROM 1
      AND ca.days_since_first_purchase > 0
      AND COALESCE(d.l365d_orders, 0) BETWEEN 50 AND 100
),
...
scored AS (
    SELECT
        c.creator_id, c.consumer_type, c.l365d_orders, c.l180d_orders, c.l28d_orders,
...
kept AS (
    SELECT creator_id, consumer_type, l365d_orders, l180d_orders, l28d_orders, v3_relative_sensitivity FROM scored   WHERE sens_decile > 1
    UNION ALL
    SELECT creator_id, consumer_type, l365d_orders, l180d_orders, l28d_orders, v3_relative_sensitivity FROM unscored
),

-- (3) Weekly rates + the >=50% recent-drop filter.
metrics AS (
    SELECT
        k.creator_id,
        k.consumer_type,
        ...
        k.l28d_orders,
        k.l180d_orders,
        k.l180d_orders / (180.0/7)  AS l180_weekly,
        k.l28d_orders / 4.0         AS l28_weekly
    FROM kept k
),
decel AS (
    SELECT *
...
-- (4) FINAL STEP (new): restrict to L180D weekly OF 0-3, return consumer-level list.
SELECT
    creator_id,
    consumer_type,
    l365d_orders,
